...
class SyncDiffusionOmni(nn.Module):
    def __init__(self, device='cuda', sd_version='2.0', hf_key=None):
        ...
        if hf_key is not None:
            ...
            model_key = "Manojb/stable-diffusion-2-1-base" # "stabilityai/stable-diffusion-2-1-base" deprecated
        elif self.sd_version == '2.0':
            model_key = "Manojb/stable-diffusion-2-base" # "stabilityai/stable-diffusion-2-base" deprecated
    ...
    def sample_syncdiffusion(
        ...
        views2 = get_views(height, width, window_size=latent_size, stride=stride, loop_closure=loop_closure)
        views = get_view_omni(height, width, window_size=latent_size, stride=stride)
        ...
        if pivot is not None:
            pivot_image = Image.open(pivot).convert("RGB") #H,W,3
            w, h = pivot_image.size
            # pivot_image = pivot_image.crop((0,0,w,h/2)) # half crop, use most sky
            pivot_image = pivot_image.resize((512,512))
            decoded_image_anchor = torch.from_numpy(np.array(pivot_image)).permute(2,0,1).unsqueeze(0).to(self.device)
        ...
        with torch.autocast('cuda'):
            for i, t in enumerate(tqdm(self.scheduler.timesteps)):
                ...
                if i < num_inference_steps * 0.6:
                    for view_idx, view_dict in enumerate(views):
                        u = view_dict["u"]
                        v = view_dict["v"]
                        grid_xy = view_dict["grid_xy"]
                        grid_xy = torch.from_numpy(grid_xy).to(self.device)

                        latent_view = latent[:, :, v, u]
                        ...
                        if (i + 1) % sync_freq == 0 and i < sync_thres:
                            ...
                            img = T.ToPILImage()(decoded_x0[0].cpu())
                            # img.save(f"latent/img{i}_{view_idx}.png")
                        ...
                        value[:, :, v, u] += latent_view_denoised
                        count[:, :, v, u] += 1

                else:
                    for view_idx, (h_start, h_end, w_start, w_end) in enumerate(views2):
                        latent_view = set_latent_view(latent, h_start, h_end, w_start, w_end)
...
class SyncDiffusion(nn.Module):
    ...
    def sample_syncdiffusion(
        ...
        if pivot is not None:
            pivot_image = Image.open(pivot) #H,W,3
            w, h = pivot_image.size
            # pivot_image = pivot_image.crop((0,0,w,h/2)) # half crop, use most sky
            pivot_image = pivot_image.resize((512,512))
            decoded_image_anchor = torch.from_numpy(np.array(pivot_image)).permute(2,0,1).unsqueeze(0).to(self.device)
